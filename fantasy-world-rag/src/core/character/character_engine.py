...
class CharacterEngine(LoggerMixin):
    ...
    async def _analyze_and_update_emotion(
        ...
        Uses simple keyword analysis for MVP. Phase 2 will use
        ...
        try:
            ...
            if include_thinking and "<thinking>" in response:
                ...
                if len(parts) > 1:
                    ...
                    response = thinking_parts[1].strip() if len(thinking_parts) > 1 else response
    ...
    def _build_prompt(
        ...
        parts = [
            ...
            "",
            self.emotional_state.generate_emotion_prompt_section(),
            "",
        ]
